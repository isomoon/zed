...
/// `sentry[tags][thread_id]` or `sentry[user][username]`, which is why the
...
mod tests {
    ...
    fn setting_a_tag_to_none_removes_it() {
        ...
        server.receive(CrashServerMessage::Tag {
            key: "sentry[tags][thread_id]".to_string(),
            value: Some("thread-1".to_string()),
        });
        server.receive(CrashServerMessage::Tag {
            key: "sentry[tags][thread_id]".to_string(),
            value: Some("thread-2".to_string()),
        });
        assert_eq!(
            ...
            BTreeMap::from([
                ...
                (
                    "sentry[tags][thread_id]".to_string(),
                    "thread-2".to_string()
                ),
            ])
        );
